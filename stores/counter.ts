...
export const useCharacterStore = defineStore('character', () => {
  ...
  async function getCharacters() {
    try {
      const character = await axios.get(baseURL + 'pokemon?limit=20&offset=0')
  ...
  async function getCharacter(value: string) {
    try {
      if (value === '') {
        ...
        const character = await axios.get(baseURL + 'pokemon/' + value)
  ...
  async function getCharacterByName(value: string) {
    try {
      if (value === '') {
        ...
        const pokemon = await axios.get(baseURL + 'pokemon/' + value)
